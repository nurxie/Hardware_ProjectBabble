...
void CameraHandler::setupBasicResolution() {
  ...
  config.frame_size = FRAMESIZE_CIF; //FRAMESIZE_240X240

  if (!psramFound()) {
    log_e("Did not find psram, setting lower image quality");
    config.fb_location = CAMERA_FB_IN_DRAM;
    config.jpeg_quality = 0; //9
    config.fb_count = 2;
    return;
  }

  log_d("Found psram, setting the higher image quality");
  config.jpeg_quality = 5;  // 0-63 lower number = higher quality, more latency
...
int CameraHandler::setCameraResolution(framesize_t frameSize) {
  if (camera_sensor->pixformat == PIXFORMAT_JPEG) {
    try {
      return FRAMESIZE_CIF; //camera_sensor->set_framesize(camera_sensor, frameSize);
    } catch (...) {
